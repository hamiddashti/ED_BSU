ed_out <- function(y1,m1,d1,y2,m2,d2,pfx){
  ...
  #             pfx: perfix of the simulation file names (e.g. for "hhh-D-2014-01-01-000000-g01.h5"; 
  #                                                       pfx is "hhh-D)
  # Output :    A dataframe containing important ED outouts
  # Example:    df<-ed_out(y1=2014,m1=01,d1=01,y2=2018,m2=12,d2=31,pfx = "hhh-D")
  #--------------------------------------------------------------------------------
  ...
  dmean_sfcw_mass_pa<-as.vector(n)
  ...
  for (i in 1:n){
    ...
    paco_id[i] <- h5read(fName[i],"/PACO_ID")                                 # Patch ID
    #H5close()
    paco_n[i] <- h5read(fName[i],"/PACO_N")                                   # Patch number per cohort
    #H5close()
    patch_n[i] <- h5read(fName[i],"/PATCH_COUNT")                             # Patch number per site
    #H5close()
    nplant[i]<-sum(h5read(fName[i],'/NPLANT'))                                # Number of plants per cohort
    #H5close()
    agb_co[i] <- mean(h5read(fName[i],'/AGB_CO'))*nplant[i];                  #[kgC/plant].....sum
    ba_co[i] <- mean(h5read(fName[i],'/BA_CO'))*nplant[i]                     # Basal area [cm2]
    balive_co[i] <-mean(h5read(fName[i],'/BALIVE'))*nplant[i]                 # Biomass alive [kg]
    bdead_co[i] <- mean(h5read(fName[i],'/BDEAD'))*nplant[i]                  # Biomass dead [kg]
    btotal[i]<- balive_co[i]+bdead_co[i]                                      # Biomass total
    bseeds_co[i] <- sum(h5read(fName[i],'/BSEEDS_CO'))                        # Biomass seeds
    daylight[i] <- h5read(fName[i],'/DAYLIGHT')/3600;                         #hours
    dbh_co [i]<- mean(h5read(fName[i],'/DBH'))                                # DBH
    ...
    deman_atm_rlong_py[i] <- h5read(fName[i],'/DMEAN_ATM_RLONG_PY');          # Atmosphere longwave radiation
    deman_atm_rshort_py[i] <- h5read(fName[i],'/DMEAN_ATM_RSHORT_PY');        # Atmospheric shortwave radiation
    dmean_available_water_pa[i] <-h5read(fName[i],'/DMEAN_AVAILABLE_WATER_PA');          # [kg/m2]
    dmean_fs_open_co[i] <-mean(h5read(fName[i],'/DMEAN_FS_OPEN_CO'))          # Net stress factor
    dmean_fsn_co[i] <-mean(h5read(fName[i],'/DMEAN_FSN_CO'))                  # Nitrogen stress factor
    dmean_fsw_co[i]<-mean(h5read(fName[i],'/DMEAN_FSW_CO'))                   # Water stress factor
    dmean_gpp_co[i]<-mean(h5read(fName[i],'/DMEAN_GPP_CO'))*nplant[i]         # GPP
    ...
    dmean_nppcroot_py[i]<-h5read(fName[i],'/DMEAN_NPPCROOT_PY')               # NPP course root
    dmean_nppfroot_py[i]<-mean(h5read(fName[i],'/DMEAN_NPPFROOT_CO'))*nplant[i]  # NPP fine root
    ...
    dmean_sfcw_depth_pa[i]<-h5read(fName[i],'/DMEAN_SFCW_DEPTH_PA')            #Daily mean - Depth - temporary water layer
    dmean_sfcw_mass_pa[i] <-h5read(fName[i],'/DMEAN_SFCW_MASS_PA')
    dmean_soil_water_pa[i] <- list(h5read(fName[i],'/DMEAN_SOIL_WATER_PA'))    # Daily mean - Soil water content
    dmean_transp_co[i] <-sum(h5read(fName[i],'/DMEAN_TRANSP_CO'))             #Daily mean - Leaf transpiration
    dmean_water_supply_co[i] <-sum(h5read(fName[i],'/DMEAN_WATER_SUPPLY_CO'))                     # Daily mean - Water supply  [kg/m2/s]  
    dmean_wshed_lg_co[i]<-h5read(fName[i],'/DMEAN_WSHED_LG_PY')               # Daily mean - Leaf shedding 
    fast_soil_c_py[i]<-h5read(fName[i],'/FAST_SOIL_C_PY')                     #Soil Carbon (Fast pool)
