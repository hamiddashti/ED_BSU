...
module io_params
  ...
  character(len=str_len)  :: afilin

  character(len=str_len) :: hfilout, afilout, pastfn, hfilin
  ! Modif. by ALF
  ...
  character(len=str_len), dimension(maxgrds) :: itoptfn,isstfn,ivegtfn,isoilfn  &
       ,ndvifn
  ! TEB
  character(len=str_len), dimension(maxgrds) :: ifusfn

  ! Modif.by ALF
  !----------------------------------------------------------------------------

  integer, parameter                               :: maxsstfiles=2000
  character(len=str_len)                           :: sstfpfx, sfcfiles, &
       topfiles
  ! TEB
  character(len=str_len)                               :: fusfiles
  ! Modif.by ALF
  character(len=str_len),dimension(maxsstfiles,maxgrds) :: fnames_sst
  ! Modif.by ALF
  ...
  character(len=str_len)                                     :: ndvifpfx
  ! Modif.by ALF
  character(len=str_len), dimension(maxndvifiles,maxgrds)     :: fnames_ndvi
  ! Modif.by ALF
