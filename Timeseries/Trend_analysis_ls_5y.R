...
ls_opt <- read.csv("ls_opt.csv", header = TRUE, as.is = TRUE)
ls_reg <- read.csv("ls_reg.csv", header = TRUE, as.is = TRUE)
ls_obs <- read.csv("ls_obs.csv", header = TRUE, as.is = TRUE)
dates <- seq(as.Date("2015/10/1"), as.Date("2017/9/30"), "days") # calibration period
ls_obs$dates <- dates
...
reg.cal.daily <- ls_reg$GPP[which(ls_reg$dates==cal_dates[1]):which(ls_reg$dates==cal_dates[length(cal_dates)])]
reg.val.daily <- ls_reg$GPP[which(ls_reg$dates==val_dates[1]):which(ls_reg$dates==val_dates[length(val_dates)])]
...
df_all_daily <- data.frame(dates,ls_obs$GPP,ls_opt$GPP,ls_reg$GPP)
colnames(df_all_daily) <- c('Date', 'Obs', "Opt","Reg")
