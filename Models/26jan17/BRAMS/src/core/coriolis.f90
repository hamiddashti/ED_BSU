...
subroutine fcorio(n2,n3,fcoru,fcorv,glat)
   ...
   real(kind=4), dimension(n2,n3) , intent(inout) :: fcoru
   real(kind=4), dimension(n2,n3) , intent(inout) :: fcorv
   real(kind=4), dimension(n2,n3) , intent(in)    :: glat
   !----- Local variables. ----------------------------------------------------------------#
   real                                           :: omega2
   integer                                        :: i
   integer                                        :: j
   ...
   do j = 1,nylast
      do i = 1,nxlast
         if ( (j+jdim > n3) .or. (i+1 > n2) ) then
            continue
         end if
         fcoru(i,j) = omega2 * sin( (glat(i,j) + glat(i+1,j)   ) * .5 * pio180 )
         fcorv(i,j) = omega2 * sin( (glat(i,j) + glat(i,j+jdim)) * .5 * pio180 )
      end do
   end do
