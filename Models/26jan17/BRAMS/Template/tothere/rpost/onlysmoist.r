...
vardf[ 1,] = c("smoist.ps","smoist" ,"Soil moisture "       ,   "m3/m3" 
              ,"imuitas" ,0.10)
#------------------------------------------------------------------------------------------#
...
latz    =   9.8
thickness = 0.60
slz       = c(-4.00, -3.00, -2.25, -1.50, -1.10, -0.80,
              -0.60, -0.45, -0.30, -0.19, -0.11, -0.05)
...
dslz   = diff(c(slz,0))
nzg    = length(dslz)
topsoil= slz >= -thickness
thickness = - slz[topsoil][1]
...
vlist = c("lon","lat","lai.ps","agb.ps","vegz0.ps","land","smoist.ps")
...
if (recomp){
   ...
   for (n in 1:nctl){
      ...
      dd$depth     = array(rep(dslz,times=dd$tmax*dd$xmax*dd$ymax),
                           dim=c(nzg,dd$tmax,dd$xmax,dd$ymax))
      dd$thick     = array(rep(thickness,times=dd$tmax*dd$xmax*dd$ymax),
                           dim=c(dd$tmax,dd$xmax,dd$ymax))
      dd$smoist.ps = aperm(a=dd$smoist.ps,perm=c(2,1,3,4))
      dd$smoist.ps = colSums(x=dd$smoist.ps[topsoil,,,]*dd$depth[topsoil,,,],dims=1
                          ,na.rm=TRUE) / dd$thick
      #------------------------------------------------------------------------------------#
