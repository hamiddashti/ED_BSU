...
mbs_opt <- read.csv("mbs_opt.csv", header = TRUE, as.is = TRUE)
mbs_reg <- read.csv("mbs_reg.csv", header = TRUE, as.is = TRUE)
...
opt.cal.daily <- mbs_opt$GPP[which(mbs_opt$dates==cal_dates[1]):which(mbs_opt$dates==cal_dates[length(cal_dates)])]
