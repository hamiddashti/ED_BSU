My_EOS <- function(df,value){
  ...
  ref_segment <- ref$curve[max.value:endday]                             
  day.ref <- ref$doy[max.value:endday]                             
  ...
  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! tr1 adn tr2 Estimated from Flux tower data !!!!!!!
  #tr1<-157 
  #tr1 <- 183       # this is from Modis 
  #tr2 <- 243
  #plot(ref_segment_df)
  ...
  #test.curve <- test[endday:max.value]
  ...
  for (eachyear in 1:n){
    test.curve <- list_daily[max.value:endday,eachyear] 
  ...
  EOS <- list("DOY1"= tp.doy1,"DOY2"= tp.doy2,"Date1"=tp.date1,"Date2"=tp.date2, "ref_tr1" = tr1,"ref_tr2" = tr2, "Daily" = list_daily)
