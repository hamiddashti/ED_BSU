...
obs_dates <- seq(as.Date(ls_obs$Date[1],"%m/%d/%Y"), as.Date(ls_obs$Date[length(ls_obs$Date)],"%m/%d/%Y"), "days") # Observation period
opt_dates <- seq(as.Date(ls_opt$dates[1],"%Y-%m-%d"), as.Date(ls_opt$dates[length(ls_opt$dates)],"%Y-%m-%d"), "days")
ls_obs$dates <- obs_dates
ls_opt$dates <- opt_dates

ls_obs_df <- data.frame(ls_obs$dates,ls_obs$GPP)
tmp <- ls_opt$GPP[which(ls_opt$dates==obs_dates[1]):which(ls_opt$dates==obs_dates[length(obs_dates)])]
ls_opt_df <- data.frame(obs_dates,tmp)
colnames(ls_opt_df) <- c("Date","GPP") 
colnames(ls_obs_df) <- c("Date","GPP") 
...
cal_dates <- seq(as.Date("2015/10/1"), as.Date("2016/9/30"), "days") # calibration period
val_dates <- seq(as.Date("2016/10/1"), as.Date("2017/9/30"), "days") # validation period
opt.cal.daily <- ls_opt$GPP[which(ls_opt$dates==cal_dates[1]):which(ls_opt$dates==cal_dates[length(cal_dates)])]
opt.val.daily <- ls_opt$GPP[which(ls_opt$dates==val_dates[1]):which(ls_opt$dates==val_dates[length(val_dates)])]
obs.cal.daily <- ls_obs$GPP[which(ls_obs$dates==cal_dates[1]):which(ls_obs$dates==cal_dates[length(cal_dates)])]
obs.val.daily <- ls_obs$GPP[which(ls_obs$dates==val_dates[1]):which(ls_obs$dates==val_dates[length(val_dates)])]
df_all_daily <- data.frame(obs_dates,ls_obs_df$GPP,ls_opt_df$GPP)
...
obs.all.monthly <- aggregate(ls_obs_df$GPP,list(format(obs_dates,"%Y-%m")),mean)
colnames(obs.all.monthly) <- c('Date', 'GPP')
opt.all.monthly <- aggregate(ls_opt_df$GPP,list(format(obs_dates,"%Y-%m")),mean)
...
obs.all.weekly <- aggregate(ls_obs_df$GPP,list(format(obs_dates,"%Y-%W")),mean)
colnames(obs.all.weekly) <- c('Date', 'GPP')
opt.all.weekly <- aggregate(ls_opt_df$GPP,list(format(obs_dates,"%Y-%W")),mean)
...
NSE.cal.obs.opt.monthly <- hydroGOF::NSE.default(obs.cal.monthly$GPP,opt.cal.monthly$GPP)
...
#tiff("N:/Data02/bcal/Personal/hamid/ED_opt/working/Manuscript/Figures/ls_calibration.tiff", units="in", width=2.7, height=2.3, res=300)
ggplot() + 
  geom_line(data = ls_obs_df, aes(x =as.Date(ls_obs_df$Date), y = ls_obs_df$GPP,color="EC tower GPP"),size=0.5) +
  geom_line(data = ls_opt_df, aes(x =as.Date(ls_opt_df$Date), y = ls_opt_df$GPP,color="Simulated GPP"),size=0.5)+
