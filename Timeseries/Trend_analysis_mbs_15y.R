...
mbs_reg <- read.csv("mbs_reg15.csv", header = TRUE, as.is = TRUE)
mbs_obs <- read.csv("mbs_obs.csv", header = TRUE, as.is = TRUE)
...
reg.cal.daily <- mbs_reg$GPP[which(mbs_reg$dates==cal_dates[1]):which(mbs_reg$dates==cal_dates[length(cal_dates)])]
reg.val.daily <- mbs_reg$GPP[which(mbs_reg$dates==val_dates[1]):which(mbs_reg$dates==val_dates[length(val_dates)])]
obs.cal.daily <- mbs_obs$GPP[which(mbs_obs$dates==cal_dates[1]):which(mbs_obs$dates==cal_dates[length(cal_dates)])]
obs.val.daily <- mbs_obs$GPP[which(mbs_obs$dates==val_dates[1]):which(mbs_obs$dates==val_dates[length(val_dates)])]
...
df_all_daily <- data.frame(dates,mbs_obs$GPP,mbs_reg$GPP)
